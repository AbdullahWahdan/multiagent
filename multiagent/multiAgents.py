...
class ReflexAgent(Agent):
    ...
    def printStats(self):
        ...
        print(f"Win rate: {self.total_wins / total_games * 100:.2f}%")
        
    
        # Outlier detection using z-score
    
    
        clear_times_np = np.array(self.clear_times)
    
        print("\n\nTiming Statistics:")
        print(f"Average win time: {np.mean(clear_times_np):.5f} sec")
        print(f"Standard deviation: {np.std(clear_times_np):.5f} sec")
        print(f"Fastest win: {np.min(clear_times_np):.10f} sec")
        print(f"Slowest win: {np.max(clear_times_np):.5f} sec")
        ...
        total_games = self.total_wins + self.total_losses
        avg_win_time = np.mean(self.clear_times) if self.clear_times else 0
        title_text = (f'Win/Loss Ratio ({self.total_wins}/{total_games} Games)\n'
                      f'Average Win Time: {avg_win_time:.2f} sec')
